...
class DQN(nn.Module):
    def __init__(self, env, lr=1e-4, gamma = 0.99, epsilon=1.0, epsilon_decay=0.995, buffer_size=500000):
        ...
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=self.lr)  #updates the weights of the model after computing gradients
    ...
    def sample_buffer(self, num_samples):
        ...
        actions = torch.as_tensor(np.array(actions, np.int64))
