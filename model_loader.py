...
batch_size = 32
lr = 1e-3
duel_network = VanillaDqn.DQN(env, lr = lr, gamma = gamma, epsilon = 0.03, buffer_size =buffer_size)
...
duel_network.eval()
duel_noise_network.load_state_dict(torch.load("saved/lunarlander_van_noise_0.05.pt"))
...
plt.savefig('noise_results/Trained_van_with_noise_0.05.png')

duel_in_noise = np.asarray(noise_duel)
duel_without_noise = np.asarray(clear_duel)
np.savetxt("rewards/van_trained_with_noise_0.05.csv",duel_in_noise,delimiter=",")
np.savetxt("rewards/van_trained_without_noise_0.05.csv",duel_without_noise,delimiter=",")