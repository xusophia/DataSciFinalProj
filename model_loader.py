...
batch_size = 32
lr = 1e-3
duel_network = VanillaDqn.DQN(env, lr = lr, gamma = gamma, epsilon = 0.06, buffer_size =buffer_size)
duel_noise_network = VanillaDqn.DQN(env, lr = lr, gamma = gamma, epsilon = 0.01, buffer_size =buffer_size)
# my_model = models.load_model("def_DQN.h5")
duel_network.load_state_dict(torch.load("saved/lunarlanderVanilla.pt"))
duel_network.eval()
duel_noise_network.load_state_dict(torch.load("saved/lunarlander_van_noise_0.5.pt"))
...
# noise_duel = run_model(duel_noise_network,noise=False)

env.close()
# gym_to_gif.save_frames_as_gif(frames)

# fig = plt.figure(figsize=(20,10))
# plt.plot([i for i in range(len(clear_duel))], clear_duel, 'y', label="Duelling trained without noise")
# plt.xlabel("Episodes")
# plt.ylabel("Rewards")
#
# plt.plot([i for i in range(len(noise_duel))], noise_duel, 'b', label="Duelling trained with noise")
# plt.legend(loc="upper left")
# plt.savefig('noise_results/Trained_duelling_with_noise_0.5.png')
#
